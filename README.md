...
- 已内置多分服兼容的数据结构与同步队列（`src/modules/roleSync`）。
- 需要在 Discord 开发者后台为 Bot 开启 `Server Members Intent`（对应 `GuildMembers` intent）。
- 可通过环境变量 `ROLE_SYNC_LINKS_JSON` 预置同步链路（示例见 `.env.sample`）。
- 已实现快慢通道调度（fast/normal），支持高频角色与基础角色不同延迟目标。
- 已实现 CSV 全流程：角色快照导出、计划文件导入、预检、应用。
- 应用阶段支持按计划自动创建缺失角色（可选视觉属性/权限复制模式）。
- 包含审计日志（90天清理）、配置快照与基础回滚能力。
- 对 Discord API 瞬时网络抖动（如 `ECONNRESET`）已加入重试与退避策略。
- 已支持手动对账（单成员/批次）、链路启停开关。
- 支持可选自动对账调度（按交集成员轮询扫描，补齐漏同步任务）。
...
- `/身份组同步 导出当前服务器角色`
- `/身份组同步 导出链路角色`
- `/身份组同步 导入计划` / `预检` / `应用` / `状态` / `快照列表` / `回滚`
- `/身份组同步 对账成员` / `对账批次` / `触发自动对账` / `设置链路状态`
- `/身份组同步 全量采集成员`（建议上线初期先跑一遍，建立交集成员基础数据）
