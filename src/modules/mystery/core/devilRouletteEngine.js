...
function defaultActionResult(action, actorId) {
    return {
        ...
        killedId: null, reloaded: false, roundEnded: false, roundWinnerId: null,
...
class DevilState {
    ...
    // 新装弹周期的先手（PVP 交替制）：= 上一周期先手的对方，彻底斩断出手权继承。
    // 先手被铐时 _setTurn 会跳过，loadFirstPlayer 记「实际行动者」而非被铐者，
    // 否则下一轮交替起点会被污染。
    _beginNewCylinderCycle() {
        const first = this.other(this.loadFirstPlayer);
        this._setTurn(first);
        this.loadFirstPlayer = this.turnPlayerId;
    }
    ...
    canUseItem(userId, itemKey) {
        ...
        if (itemKey === 'handcuffs' && this.handcuffed.has(this.other(userId))) return false; // 对手已被铐住
    ...
    _smartStealChoice(userId, stealable) {
        // 庄家偷道具偏好：残血优先回复，其次进攻性道具，最后随机。只用公开信息。
        const maxHp = GAME_CONFIG.hp;
