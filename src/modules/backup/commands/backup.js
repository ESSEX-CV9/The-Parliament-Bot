...
async function execute(interaction) {
  if (!checkAdminPermission(interaction.member)) {
    return interaction.reply({ content: '❌ 你没有权限执行备份操作。', ephemeral: true });
  }

  const guildId = interaction.guildId;
  if (activeBackups.has(guildId)) {
    return interaction.reply({ content: '⏳ 当前服务器已有备份任务在运行，请等待完成。', ephemeral: true });
  }
  ...
  try {
    ...
    return interaction.reply({ content: '❌ 无效的目标 URL。', ephemeral: true });
  }
  ...
  if (targetId) {
    ...
    if (!target) {
      return interaction.reply({ content: `❌ 找不到 ID 为 \`${targetId}\` 的频道或分类。`, ephemeral: true });
    }
    if (type === 'category' && target.type !== ChannelType.GuildCategory) {
      return interaction.reply({ content: `❌ ID \`${targetId}\` 不是一个分类频道。`, ephemeral: true });
    }
    if (type === 'channel' && target.type === ChannelType.GuildCategory) {
      return interaction.reply({ content: `❌ ID \`${targetId}\` 是一个分类，请使用 \`/backup category\`。`, ephemeral: true });
    }
  }
