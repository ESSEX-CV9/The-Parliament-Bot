...
async function executeDeleteMessage(client, voteData) {
    try {
        ...
        console.log(`开始执行删除消息: ${targetMessageId}, 反应数量: ${currentReactionCount}`);
        ...
        if (!message) {
            throw new Error(`找不到消息: ${targetMessageId}`);
        }
...
async function executeMuteUser(client, voteData) {
    try {
        const { guildId, targetChannelId, targetMessageId, targetUserId, currentReactionCount, executedActions = [] } = voteData;
        
        console.log(`开始执行禁言用户: ${targetUserId}, 反应数量: ${currentReactionCount}`);
...
async function checkAndDeleteUserMessage(client, voteData) {
    try {
        const { guildId, targetChannelId, targetMessageId } = voteData;
