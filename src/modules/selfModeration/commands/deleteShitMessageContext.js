// src\modules\selfModeration\commands\deleteShitMessageContext.js
...
/**
 * 「删除搬屎消息」的右键入口。
 * 行为与斜杠指令 /删除搬屎消息 完全一致，只是消息链接直接取自被右键的消息，
 * 不需要用户手动复制粘贴。
 */
...
async function execute(interaction) {
    try {
        ...
        const preCheck = await runSelfModerationPreChecks(interaction, 'delete');
        if (!preCheck.ok) {
        ...
        console.log(`用户 ${interaction.user.tag} 在频道 ${interaction.channel.name} 通过右键发起删除消息投票`);
        ...
        console.error('执行右键删除搬屎消息指令时出错:', error);
