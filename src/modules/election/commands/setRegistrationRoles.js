...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('设置')
                .setDescription('设置允许报名的身份组')
                .addRoleOption(option =>
                    option.setName('身份组1')
                        .setDescription('允许报名的身份组')
                        .setRequired(true))
                .addRoleOption(option =>
                    option.setName('身份组2')
                        .setDescription('允许报名的身份组（可选）')
                        .setRequired(false))
                .addRoleOption(option =>
                    option.setName('身份组3')
                        .setDescription('允许报名的身份组（可选）')
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('查看')
                .setDescription('查看当前的报名权限配置'))
        .addSubcommand(subcommand =>
            subcommand
                .setName('清空')
                .setDescription('清空报名权限限制（允许所有人报名）'))
    ...
    async execute(interaction) {
        try {
            ...
            if (subcommand === '查看') {
                ...
                // 获取身份组名称
                const roleNames = [];
                for (const roleId of permissions.registrationRoles) {
            ...
            if (subcommand === '清空') {
                await ElectionPermissions.clearRegistrationRoles(guildId);
            ...
            if (subcommand === '设置') {
                ...
                // 保存权限配置
                await ElectionPermissions.saveRegistrationRoles(guildId, selectedRoles);
